function [A1, A2, A3] = func_GetAx(y, Fs, F0, F1, F2, F3, sampleshift, N_periods)
...
%             [A1_, ~] = ana_GetMagnitudeMax(yseg, F1(k), Fs, 8192);
%             [A2_, ~] = ana_GetMagnitudeMax(yseg, F2(k), Fs, 8192);
%             [A3_, ~] = ana_GetMagnitudeMax(yseg, F3(k), Fs, 8192);
%             A1(k) = A1_;
%             A2(k) = A2_;
%             A3(k) = A3_;
            
            As = ana_GetMagnitudeMax(yseg, [F1(k) F2(k) F3(k)], Fs, 8192);
            A1(k) = As(1);
            A2(k) = As(2);
            A3(k) = As(3);
...
%------------------------------------------------------------------------
function [M] = ana_GetMagnitudeMax(x, Fx, Fs, fftlen)
...
%       highf=Fs/2-fstep; 
%   end
  highf( highf>Fs/2-fstep) = Fs/2-fstep;

  
  for cnt = 1:length(Fx)
      X_ = X(1+round(lowf(cnt)/fstep):1+round(highf(cnt)/fstep), :);
      X_ = 20*log10(factor* abs(X_));
      [M(cnt),~] = max(X_);
