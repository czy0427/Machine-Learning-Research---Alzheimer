function [isVoiced, F0, F1, F2, F3, F4, B1, B2, B3, B4,err] = func_Praat_wb(wavfile, windowlength, frameshift, datalen, params, dirPraat)
%
% Author: Yen-Liang Shue, Speech Processing and Auditory Perception Laboratory, UCLA
% Modified by Kristine Yu 2010-10-16
% Modified by Soo Jin Park 2016-11-03
...
if ispc
    ...
    cmd_v = sprintf(['"%sPraat_windows.exe" --run %spraatF0.praat "%s" %.3f ' ...
        ...
        dirPraat, dirPraat, wavfile, frameshift, minF0, maxF0, ...
    ...
    cmd_fmt = sprintf('"%sPraat_windows.exe" --run %spraatformants.praat "%s" %.3f %.3f %.1f %d', ...
        dirPraat, dirPraat, wavfile, frameshift, windowlength, num_formants, maxFormant);

elseif (ismac) % mac osx can run Praat using terminal, call Praat from
                % Nix/ folder
    UNIX=true;

    if ~exist([dirPraat 'Praat.app'], 'file')
        fprintf('Praat not found: please install Praat through the Settings menu\n')
        err=-1;
    else
        curr_wav = wavfile;
        
        cmd_v = sprintf(['%sPraat.app/Contents/MacOS/Praat %spraatF0.praat ' ...
            '%s %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %d %d %.3f %d %s'], ...
            dirPraat,dirPraat, curr_wav, frameshift, minF0, maxF0, silthres, voicethres, octavecost, octavejumpcost, voiunvoicost, killoctavejumps, smooth, smoothbw, interpolate, method);
        
        
        cmd_f0 = sprintf(['%sPraat.app/Contents/MacOS/Praat %spraatF0.praat ' ...
            '%s %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %d %d %.3f %d %s'], ...
            dirPraat,dirPraat, curr_wav, frameshift, minF0, maxF0, 0, 0, octavecost, octavejumpcost, voiunvoicost, killoctavejumps, smooth, smoothbw, interpolate, method);

        cmd_fmt = sprintf(['%sPraat.app/Contents/MacOS/Praat %spraatformants.praat ' ...
        '%s %.3f %.3f %.1f %d'], ...
        dirPraat, dirPraat, curr_wav, frameshift, windowlength, num_formants, maxFormant);
    end
    

elseif isunix
    % Do not forget to chmod Praat_linux file (755)
    
    UNIX = true;
    
    cmd_v = sprintf(['%sPraat_linux %spraatF0.praat %s %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %d %d %.3f %d %s'], ...
        dirPraat, dirPraat, wavfile, frameshift, minF0, maxF0, silthres, voicethres, octavecost, octavejumpcost, voiunvoicost, killoctavejumps, smooth, smoothbw, interpolate, method);
    
    
    cmd_f0 = sprintf(['%sPraat_linux %spraatF0.praat %s %.3f %.3f %.3f %.3f %.3f %.3f %.3f %.3f %d %d %.3f %d %s'], ...
        dirPraat, dirPraat, wavfile, frameshift, minF0, maxF0, 0, 0, octavecost, octavejumpcost, voiunvoicost, killoctavejumps, smooth, smoothbw, interpolate, method);
    
    cmd_fmt = sprintf(['%sPraat_linux %spraatformants.praat ' ...
        '%s %.3f %.3f %.1f %d'], dirPraat, dirPraat, wavfile, frameshift, windowlength, num_formants, maxFormant);
...
if err % oops, error, exit now
    ...
    %
    fprintf('error occurred during running Praat')
    return;
end
